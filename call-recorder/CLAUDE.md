...
- **Oportunidades** -> `create_idea.py --status "em analise" --origin <transcricao>`.
  ...
  entre conversas.
- **Cruzar antes de criar**: _reports/Action-Dashboard.md + backlog. Recorrencia
...
**Output (vault):**
- Session note: `Areas/English-Learning/sessions/YYYY-MM-DD_english-coach.md` — **um arquivo por DIA desde 2026-09-02**, com uma secao `## Call N - HH:MM` por call e `calls: N` no frontmatter. Era um arquivo por call; o coach dispara por IDIOMA, entao um dia com 4 calls em ingles gerava 4 relatorios e 4 linhas no progress, e o grafico passava a medir frequencia de reuniao. O frontmatter do dia (`level`, `overall`) carrega a call mais recente, porque `_level_history` le so os primeiros 600 caracteres.
...
| `oneonone`, `manager`, `agenda` | **Gateway** (same fallback) | Revisto pelo Kelvin em 2026-09-02, decisao "A parcial": o Ollama segurava 4,2 GB numa maquina de 16 GB que ficava com 300-500 MB livres. Ate esse dia o `process.py` chamava o Ollama direto e a allowlist nao governava nada dele — o que esta tabela dizia era falso |
| `note`, `capture`, `transcript` | **Ollama, sempre** | `note` e `capture` sao o Inbox, onde cai o pessoal dele (visto, pensao, ida para a Alemanha). O ADR `2026-08-31-sistema-de-estudo-mdm.md`, decisao 4: conteudo da transicao nunca roda no gateway, *porque o gateway e logado pelo empregador*. `transcript` e o resumo de contexto do coach, que dispara por idioma e alcanca 1:1 |
| `coach-weekly` | **Ollama, sempre** (desde 2026-09-10) | Consolidado semanal `agent/english_coach.py` (segunda 08:30). Le PASTA INTEIRA, `Inbox/` incluso — nao e o mesmo caso de `coach`, que le so a fala do Kelvin numa call. Ate 09/09 a garantia era acidental: URL fixa do Ollama, ou seja "nao ha caminho" em vez de "o caminho e recusado". Travado em `tests/test_english_coach_local.py` |
...
| `coach_grammar.py` | **Inventario de gramatica por registro (2026-09-09).** Consumidor que faltava do `coach_patterns`: o `detect()` ja rodava em toda sessao e o resultado so virava linha de prompt, descartado depois. Agrega duas fontes e escreve `{COACH_DIR}/grammar.json`, lido pelo painel de estudo. Registros NUNCA sao somados: `fala` (transcript das calls, valido em qualquer data) e `escrito-informal` (prompts do Kelvin em `~/.claude/projects`, que e ele digitando). Escrito profissional fica de fora: sem acesso, e desde jun/2026 esta contaminado por AI pela regra do voice-gate. **Grava `quote` (o span casado, 1-2 palavras), nunca `context`** - o `detect()` tambem devolve 60 chars de cada lado, e trocar um pelo outro poria trecho de conversa pessoal num arquivo do vault. O tracker declara o que NAO mede (typo, pontuacao, registro profissional): numero confiante sobre o que o detector nao ve e pior que numero nenhum. `python coach_grammar.py` reconstroi; `--so-fala` pula a varredura dos transcritos. |
