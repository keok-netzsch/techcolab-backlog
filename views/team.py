        ...
        with st.expander(
            ...
            if st.button("📋 Generate agenda", key=f"tm_btn_{_tm['folder']}"):
                with st.spinner("Generating agenda via Ollama…"):
                    try:
                        _okr_txt = ((_folder / "OKR.md").read_text(encoding="utf-8", errors="replace")[:700]
                                    if (_folder / "OKR.md").exists() else "")
                        _pdi_txt = ((_folder / "PDI.md").read_text(encoding="utf-8", errors="replace")[:600]
                                    if (_folder / "PDI.md").exists() else "")
                        _ctx = ""
                        if _sess:
                            _s0 = _sess[0]
