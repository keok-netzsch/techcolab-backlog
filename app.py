...
# Enabled only when APP_PASSPHRASE_HASH is set (via Settings page or env var).
# Hash format: "pbkdf2$<hex-salt>$<hex-hash>" — generated by settings.py.
from config import APP_PASSPHRASE_HASH as _GATE_HASH

def _check_passphrase(entered: str, stored_hash: str) -> bool:
    try:
        _, salt_hex, hash_hex = stored_hash.split("$")
        salt = bytes.fromhex(salt_hex)
        expected = bytes.fromhex(hash_hex)
        actual = hashlib.pbkdf2_hmac("sha256", entered.encode(), salt, 200_000)
        return actual == expected
    except Exception:
        return False

if _GATE_HASH:
    if not st.session_state.get("_gate_ok"):
        st.markdown(
            "<style>section[data-testid='stSidebar']{display:none}</style>",
            unsafe_allow_html=True,
        )
        st.markdown("## Personal Toolkit · Techco.lab")
        _entered = st.text_input("Passphrase", type="password", key="_gate_input")
        if st.button("Unlock", type="primary"):
            if _check_passphrase(_entered, _GATE_HASH):
