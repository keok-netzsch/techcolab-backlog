...
| `analise-de-fontes` | daily 17:00 (**since 2026-09-10**) | Claude scheduled routine (nao e Task Scheduler) — fecha a analise de ate 2 fontes externas por dia. Le o texto por `analyse --show` (nota, sidecar ou cache local, sem timestamps), roda `brief` no tema ANTES de escrever para poder confrontar, e aplica com `analyse --apply`. Cada impact vira proposta em Concepts/_proposals/, e a rotina nao aprova nenhuma. Silenciosa quando nao ha pendencia. 17:00 fica antes do vault-daily-commit (17:30) e do indice (18:00), entao o que ela escreve entra no mesmo dia. Existe porque no dia em que o cano ficou pronto eram 16 fontes com 14 sem analise: trazer nunca foi o gargalo |
| `dossie-do-dia` | weekdays 07:45 (**since 2026-09-10**) | Claude scheduled routine (nao e Task Scheduler) — le a agenda do dia pelo conector de calendario e chama `python -m vaultsources dossier "<Nome>"` para cada reuniao com pessoa que o vault conhece. Entrega no chat, no maximo 6 linhas por reuniao. Silenciosa em dia sem reuniao com pessoa conhecida. Existe porque o vault so respondia quando perguntado: 1129 notas e nenhum momento em que aquilo chegava sozinho. Le apenas — nao cria nem edita nota |
...
| `TechColab Aprovacoes Pendentes` | weekdays 08:40 (**since 2026-09-04**) | `notify.ps1 -Profile aprovacoes-pendentes` → `scripts/notify-body/aprovacoes-body.ps1` — open ledger items (count, how many high-priority, how many older than 14 days, the 3 oldest by subject) plus, as a safety net, any Team Memory week with a draft and no approval. Silent when both are clean. Fires 10 min after the `pendencias-do-kelvin` routine builds the panel: the panel lives inside Claude and only exists if he opens the app, so this is what makes him open it. Born from his ask on 2026-09-04 ("essas tasks de aprovação que são recorrentes estejam em rotinas, com o lembrete no horário") after `10_2ndBrain/Team Memory/` was found empty since the 28/08 migration — the weekly approval had no ledger entry, so no routine ever showed it, and the only way to approve was editing frontmatter in Obsidian, which pattern 1 forbids |
...
| `team-memory-alerta` | `7 11 * * 1-5` | weekdays 11:07 | Checks whether the Team Memory Agent captured and drafted today; speaks only on failure. **Since 2026-09-04** it also registers the weekly approval in the pending ledger (once per week, idempotent) so it reaches Kelvin through the panel instead of the Obsidian frontmatter edit it used to require |
